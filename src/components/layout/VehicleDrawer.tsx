...
export function VehicleDrawer() {
  ...
  const devices = Array.isArray(devicesQuery?.data)
    ? devicesQuery.data
    : Array.isArray(devicesQuery?.data?.devices)
    ? devicesQuery.data.devices
    : Array.isArray(devicesQuery?.data?.data)
    ? devicesQuery.data.data
    : [];

  const positions = Array.isArray(positionsQuery?.data)
    ? positionsQuery.data
    : Array.isArray(positionsQuery?.data?.positions)
    ? positionsQuery.data.positions
    : Array.isArray(positionsQuery?.data?.data)
    ? positionsQuery.data.data
    : [];

  // DEBUGGING
  console.log("DEVICES QUERY:", devicesQuery);
  console.log("POSITIONS QUERY:", positionsQuery);
  ...
  const position =
    positions.find((p: any) =>
      p?.vehicleId
        ? String(p.vehicleId) === String(selectedDeviceUid)
        : false
    ) ??
    positions.find(
      (p: any) =>
        ...
        p?.plateNumber?.toUpperCase?.() ===
          device?.plate_number?.toUpperCase?.()
    );

  if (!selectedDeviceUid || !device) {
    return null;
  }
  ...
  const isOnline = tsRaw
    ? Date.now() - new Date(tsRaw).getTime() <
      3 * 60_000
    : false;
        ...
        <div className="space-y-4 px-4 py-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-lg bg-muted p-3">
              ...
              <p className="text-sm font-semibold">
                {position
                  ? Number(position?.heading ?? 0).toFixed(0)
                  : "—"}
                °
              </p>
            </div>
